...
function loadLoginView(){
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4 ){
			if(xhr.status == 200){
				...
				$('#view').html(xhr.responseText);//set innerhtml of div to be
				//xhr.responseText which is the login.html we just got as a
				//response from the LoadLoginViewServlet.java.
	...
	xhr.open('GET', 'login.view');// 'loadLogin');//we will open our request and
	//the request will be a GET request and we wanna send it to /loadLogin
	xhr.send();//then send. since get request, no request body
}
